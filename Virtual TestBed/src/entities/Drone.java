...
import static tools.Tools.printVector;
...
public class Drone{
	...
	public void timePassed(float timePassed){
		...
		Vector3f extraVel = ignoreSmallValues(transformVector(droneToWorld, scaleVector(velocityAcceleration1, timePassed)));
	
		Vector3f normalVel = addVectors(extraVel, velocity);
		Vector3f counterVel = ignoreSmallValues(transformVector(droneToWorld, scaleVector(velocityAcceleration2, timePassed)));
		...
		Vector3f angularAcceleration = ignoreSmallValues(new Vector3f(totalMoment.x/inX, totalMoment.y/inY, totalMoment.z/inZ));
		...
		if(isCrashed()){
			throw new RuntimeException(toString() + ": Crashed because it touched the ground");
		}
	}
	...
	public static void addModels(TexturedModel model, TexturedModel shadowModel) {
		Drone.model = model;
	...
	private Vector3f ignoreSmallValues(Vector3f vector) {
		float eps = 0.0001f;
	...
	public Vector3f getCameraPosition() {
		...
		Vector3f cameraPos = new Vector3f(0,50*(float)Math.cos(Math.PI/6),50*(float)Math.sin(Math.PI/6));
